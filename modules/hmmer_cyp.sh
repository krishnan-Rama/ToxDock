...
CYP450_HMM_URL="https://www.ebi.ac.uk/interpro/wwwapi//entry/pfam/PF00067?annotation=hmm"
HMM_FILE="${WORKINGDIR}/cyp_pfam_hmm.hmm"
...
# Download and prepare the CYP450 HMM file if not already present
if [ ! -f "$HMM_FILE" ]; then
    echo "Downloading Cytochrome P450 HMM file..."
    wget -O ${HMM_FILE}.gz "$CYP450_HMM_URL"
    gunzip "${HMM_FILE}.gz"
fi
...
for header in $headers; do
    ...
    # Run hmmsearch on the extracted sequence with the Cytochrome P450 HMM profile
    hmmsearch --tblout temp_results.txt "${HMM_FILE}" temp_seq.fasta
